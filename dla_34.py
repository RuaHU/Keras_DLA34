...
class BasicBlock():
    ...
    def __call__(self, x, residual=None):
        ...
        if self.stride == 2:
            ...
            padding = 'valid'
        x = KL.Conv2D(self.channels, 3, name = self.name + '.conv1', strides = self.stride, padding = padding, bias = False, dilation_rate = self.dilation)(x)
        x = KL.BatchNormalization(epsilon=1e-5, name = self.name + '.bn1')(x)
        x = KL.Activation('relu')(x)
        x = KL.Conv2D(self.channels, 3, name = self.name + '.conv2', strides = 1, padding = 'same', bias = False, dilation_rate = self.dilation)(x)
